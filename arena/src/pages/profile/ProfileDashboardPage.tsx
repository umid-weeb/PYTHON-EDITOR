        ...
        <div className="space-y-6">
              ...
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 text-sm text-arena-muted">{profile.display_name ? profile.display_name : " "}</div>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <DifficultyBar label="Easy" solved={problemTotals.easy.solved} total={problemTotals.easy.total} colorClass="bg-emerald-400" />
              <DifficultyBar label="Medium" solved={problemTotals.medium.solved} total={problemTotals.medium.total} colorClass="bg-amber-400" />
              <DifficultyBar label="Hard" solved={problemTotals.hard.solved} total={problemTotals.hard.total} colorClass="bg-rose-400" />
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Link className="text-xs font-semibold text-arena-primaryStrong hover:underline" to={`/profile/${encodeURIComponent(username)}/submissions`}>
                View all
              </Link>
            </div>
            ...
            <div className="divide-y divide-white/5 overflow-hidden rounded-xl border border-white/10 bg-[#0b1220]">
              {submissions.slice(0, 8).map((submission, index) => {
                const statusLower = String(submission.status || submission.verdict || "").toLowerCase();
                ...
                return (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      <div
                        className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${
                          isAccepted ? "bg-emerald-500/20" : "bg-rose-500/20"
                        }`}
                      >
                        {isAccepted ? (
                    ...
                    >
                      {isAccepted ? "Accepted" : submission.status || submission.verdict || "--"}
                    </div>
                  </div>
                );
              })}
              ...
              {submissions.length === 0 ? (
                <div className="px-4 py-8 text-center text-sm text-arena-muted">
                  {isOwnProfile ? "No submissions yet. Start solving problems!" : "This user has no recorded submissions yet."}
                </div>
              ) : null}
            </div>
          </section>
